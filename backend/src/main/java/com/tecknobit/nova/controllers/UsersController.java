...
import com.tecknobit.nova.Launcher;
...
public class UsersController extends NovaController {
    ...
    public String signUp(@RequestBody Map<String, String> payload) {
        ...
        if(Launcher.protector.serverSecretMatches(jsonHelper.getString(SERVER_SECRET_KEY))) {
            ...
            if(isPasswordValid(password)) {
                if(isLanguageValid(language)) {
                    ...
                    if(personalData.length == 2) {
                        id = Launcher.generateIdentifier();
                        token = Launcher.generateIdentifier();
